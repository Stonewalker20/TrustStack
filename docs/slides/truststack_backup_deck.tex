...
\title{TrustStack}
\subtitle{A Local-First, Evaluation Standard for Trustworthy AI}
...
\begin{frame}{3. Data}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{block}{Live evidence path}
      ...
      \begin{itemize}
        ...
        \item Preloaded ABB, Siemens, and FANUC manuals from a Kaggle technical-manuals dataset
    ...
    \begin{block}{Benchmark path}
      ...
      \begin{itemize}
        ...
        \item Public slices from SciFact (Wadden et al., 2020) and HotpotQA (Yang et al., 2018)
  ...
  \end{center}
  \vspace{0.15em}
  {\tiny\color{tsmuted}Demo source citation: Kaggle technical manuals dataset by Singh (2025), used for the preloaded evidence dropdown.}
\end{frame}

\begin{frame}{4. Methods and System Architecture}
  \slidelead{The system follows a simple five-step pipeline so users can see where trust is gained or lost, including a reproducible preloaded-source path and calibrated prompt ladder for live demos.}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{block}{Implementation}
      ...
      \begin{itemize}
        ...
        \item Sample prompts are scored before display
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
...
\begin{frame}{5. TrustStack Evaluation Standard v2.0}
  ...
  \end{columns}
  \vspace{0.25em}
  {\small\color{tsmuted}Why it matters: users can see why confidence changed, what support is missing, and when review is required.}
\end{frame}

\begin{frame}{6. Synthetic Benchmark Findings}
...
\vspace{0em}
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.94,transform shape]
  \fill[tsteal!28] (0,0) rectangle (2.45,0.24);
  \fill[ougold!35] (2.8,0) rectangle (4.75,0.24);
  \fill[tswarn!28] (5.1,0) rectangle (7.05,0.24);
  \node[anchor=south,font=\tiny] at (1.225,0.27) {Strong};
  \node[anchor=south,font=\tiny] at (3.775,0.27) {Mixed};
  \node[anchor=south,font=\tiny] at (6.075,0.27) {Weak};
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{frame}{7. External Benchmark Findings}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{block}{What this demonstrates}
      The system now runs on 18 checked-in public examples, not only internal tests.
    \end{block}
    \begin{block}{Interpretation}
      Traceability is already strong. Retrieval and answer quality remain the next clear optimization target.
    \end{block}
    \column{0.48\textwidth}
    \statcard{Aggregate TrustStack score}{67.11 / 100}
    \vspace{0.45em}
    \statcard{Task metric}{0.009}
    \vspace{0.45em}
    \statcard{Citation alignment}{93.4\%}
  \end{columns}
\vspace{0.25em}
{\scriptsize\color{tsmuted}Benchmarks cited in the report: SciFact (Wadden et al., 2020) and HotpotQA (Yang et al., 2018).}
\end{frame}

\begin{frame}[shrink=2]{8. Validity and Next-Stage Readiness}
  \slidelead{The current version is a strong baseline: measurable, reproducible, and already exposing the next evaluator bottleneck clearly.}
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{block}{What is already strong}
      ...
      \begin{itemize}
        ...
        \item Measured prompt calibration per document
      \end{itemize}
    \end{block}
    \column{0.49\textwidth}
    \begin{block}{What comes next}
      \small
      \begin{itemize}
        \item stronger penalties for unsupported specificity
  ...
  \end{columns}
\vspace{0.2em}
{\scriptsize\color{tsmuted}Current finding: strong and review-band prompts calibrate reliably, but forcing the lowest band still depends more on evaluator tuning than on prompt wording alone.}
\vspace{0.35em}
\begin{center}
